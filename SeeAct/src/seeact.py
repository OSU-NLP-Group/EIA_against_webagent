...
async def main(config, base_dir) -> None:
    ...
    ranker_path = None
    try:
        ranker_path = os.path.join(base_dir, config["experiment"]["ranker_path"]) if not os.path.isabs(
            config["experiment"]["ranker_path"]) else config["experiment"]["ranker_path"]
        if not os.path.exists(ranker_path):
            print("no ranker at: ",ranker_path)
            ranker_path = None
    ...
    for single_query_task in query_tasks:
        ...
        save_file_dir = os.path.join(save_file_dir,config["element_des"])
        save_file_dir = os.path.join(save_file_dir,config["basic"]["task_name"])
        ...
        async with async_playwright() as playwright:
            session_control.browser = await normal_launch_async(playwright,headless=config["headless"])
            ...
            while not complete_flag:
                ...
                logger.info('-' * 10)
                elements = await get_interactive_elements_with_playwright(session_control.active_page,config)

                if tracing:
                    await session_control.context.tracing.start(screenshots=trace_screenshots, snapshots=trace_snapshots,
                ...
                if len(elements) == 0:
                    ...
                    if tracing:
                        ...
                        await session_control.context.tracing.stop()
                ...
                for multichoice_i in range(0, num_choices, step_length):
                    ...
                    if dev_mode:
                        logger.info(
                            "Viewport Height： " + str(viewport_height) + "   ------   " + "height start： " + str(
                                height_start))
                        logger.info("Height End： " + str(height_end))
...
if __name__ == "__main__":
    ...
    parser.add_argument("-c", "--config_path", help="Path to the TOML configuration file.", type=str, metavar='config',
                        default=f"{os.path.join('config', 'simple.toml')}")
    parser.add_argument("--element_des",choices=["default","aria-label-button"],default="default",help="affect the element description")
    ...
    config["element_des"] = args.element_des
    config["headless"] = not args.head
