...
async def normal_launch_async(playwright: Playwright,trace_dir=None,headless=False):
...
async def get_element_description_original(element, tag_name, role_value, type_value):
    '''
...
async def get_element_description_aria_label(element, tag_name, role_value, type_value):
    '''
         Asynchronously generates a descriptive text for a web element based on its tag type.
         Handles various HTML elements like 'select', 'input', and 'textarea', extracting attributes and content relevant to accessibility and interaction.
    '''
...
async def get_element_data(element, tag_name,config):
    ...
    id_value = await element.get_attribute('id', timeout=0)
    # await aprint("start to get element description",element,tag_name )
    if config["element_des"] == "default":
        description = await get_element_description_original(element, real_tag_name, role_value, type_value)
    elif config["element_des"] == "aria-label-button":
        description = await get_element_description_aria_label(element, real_tag_name, role_value, type_value)
    else:
...
async def get_interactive_elements_with_playwright(page,config):
    ...
    for selector in interactive_elements_selectors:
        ...
        for index in range(element_count):
            ...
            task = get_element_data(element, tag_name, config)
